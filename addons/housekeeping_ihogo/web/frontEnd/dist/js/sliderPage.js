    ...
    methods: {
        deleteImg: function (id) {
            ...
            if (window.confirm('你确定要删除吗?')) {
                $.ajax({
                    type: "GET",
                    url: rooturl + 'backEnd/public/?s=SliderImg.delete',
                    data: {
                        id: id
                    },
                    dataType: "json",
...
function getAllData() {
    $.ajax({
        type: "GET",
        url: rooturl + 'backEnd/public/?s=SliderImg.getAllData',
        data: {
            ...
            $('#imgUpForm').ajaxSubmit({
                success: function (resData) {
                    if (resData.ret == 200) {
                        ...
                        $.ajax({
                            type: "GET",
                            url: rooturl + 'backEnd/public/?s=SliderImg.insert',
                            data: {
                                img: imgurl,
                                sort: $('#exampleInputPassword1').val() ? parseInt($('#exampleInputPassword1').val()) : '',
                                url: $('#inputTitle').val() ? $('#inputTitle').val() :1,
                            },
                            dataType: "json",
                            success: function (datar) {
                                getAllData();
